...
public class Repository {
    private static final Logger LOG = Logger
	    .getLogger(Repository.class.getName());

    private PersistenceManager mPm = PMF.get().getPersistenceManager();

    public Twitter getVerifiedTwitter(String accessToken)
	    throws TwitterException {
	Twitter twitter = mPm
		.getObjectById(TwitterAccessToken.class, accessToken)
		.getTwitter();
    ...
    public void getHomeTimeline(Twitter twitter, List<Status> dest)
	    throws TwitterException {
    ...
    private void setNextSinceId(Twitter twitter, long nextSinceId)
	    throws TwitterException {
    ...
    public void getFollowers(Twitter twitter, Set<Long> followers)
	    throws TwitterException {
	...
	List<Tweet> tweetids = (List<Tweet>) query
		.execute(String.valueOf(cal.getTime().getTime()));
	...
	try {
	    ...
	    LOG.info("dairy: getMax_favnum(): " + day.getMax_favnum() + "\n"
		    + "https://twitter.com/exsample/status/" + day.getMax_id());
	    if (status_favmax.getFavoriteCount() > day.getMax_favnum()) {
		day.setTweetFav(status_favmax.getId(),
			status_favmax.getFavoriteCount());
	    }
	} catch (Exception e) {
	    TweetFav day = new TweetFav("dairy", status_favmax.getId(),
		    status_favmax.getFavoriteCount());
	...
	str += "(" + (cal.get(Calendar.MONTH) + 1) + "/"
		...
		+ status.getUser().getScreenName() + " #MaikataDailyFav";
    ...
    public void updateStatus(Twitter twitter, StatusUpdate latestStatus)
	    throws TwitterException {
	twitter.updateStatus(latestStatus);
    }

    public void reply(Twitter twitter, Status status, String string)
	    throws TwitterException {
